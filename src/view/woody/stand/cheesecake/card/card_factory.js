 ...
 *
 * @param oHistoryItem
 * @returns {Cotton.UI.Stand.Cheesecake.Card.Card}
 */
// FIXME(rmoutard) : create a real factory.
Cotton.UI.Stand.Cheesecake.Card.Factory = Class.extend({
  ...
  get : function(oHistoryItem, oGlobalDispatcher) {
    ...
    if (oUrl.searchImage || reg.exec(oHistoryItem.url()) && !oUrl.pathname.match(fileReg)) {
      ...
      return new Cotton.UI.Stand.Cheesecake.Card.Image(sImageUrl, oHistoryItem, oGlobalDispatcher);
    } else if (oUrl.hostname === "www.youtube.com" && oUrl.dSearch['v']) {
      // Video - Youtube
      return new Cotton.UI.Stand.Cheesecake.Card.Video(oUrl.dSearch['v'], "youtube",
        oHistoryItem, oGlobalDispatcher);
    } else if (oUrl.hostname === "vimeo.com"
        && oUrl.pathname.match(/(\/[0-9]+)$/)) {
      // Video - Vimeo
      return new Cotton.UI.Stand.Cheesecake.Card.Video(sLastStringFromPathname, "vimeo",
        oHistoryItem, oGlobalDispatcher);
    } else if (oUrl.hostname === "www.dailymotion.com"
        && oUrl.pathname.split('/')[1] == "video") {
      // Video - Dailymotion
      return new Cotton.UI.Stand.Cheesecake.Card.Video(oUrl.pathname.split('/')[2],
        "dailymotion", oHistoryItem, oGlobalDispatcher);
    } else if (oUrl.hostname === "www.dailymotion.com" && oUrl.dHash['video']) {
      return new Cotton.UI.Stand.Cheesecake.Card.Video(oUrl.dHash['video'],
        "dailymotion", oHistoryItem, oGlobalDispatcher);
    } else if (oUrl.hostname.match(/^(maps\.google\.)/)
        && oUrl.pathname == "/maps") {
      var sMapCode = oUrl.dSearch['q'];
      return new Cotton.UI.Stand.Cheesecake.Card.Map(sMapCode, oHistoryItem, oGlobalDispatcher);
      ...
      if (oUrl.dSearch['q']){
        ...
        // Default
        return new Cotton.UI.Stand.Cheesecake.Card.Default(oHistoryItem, oGlobalDispatcher);
      }
    } else {
      // Default
      return new Cotton.UI.Stand.Cheesecake.Card.Default(oHistoryItem, oGlobalDispatcher);
