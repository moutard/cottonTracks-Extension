...
/**
 * For each oVisitItem in lVisitItems, extract words of the url.
 * 
 * @param {Object}
 *          lVisitItems : need oUrl.
 * @returns
 */
Cotton.Algo.PreTreatment.computeExtractedWordsFromUrl = function(lVisitItems) {
  // Instead of computing every time you compute a distance

  for ( var i = 0; i < lVisitItems.length; i++) {
    // TODO(rmoutard) : improve this regexp
    var oRegExp = new RegExp('(\/||-||_)?([a-zA-Z0-9]*)&?', 'g');

    var oRegExpResult = oRegExp.exec(lVisitItems[i]['oUrl']['pathname']);
    while (oRegExpResult[2] != "" && oRegExpResult[2] != undefined) {
      lVisitItems[i]['lExtractedWords'].push(oRegExpResult[2]);
      oRegExpResult = oRegExp.exec(lVisitItems[i]['oUrl']['pathname']);
    }
  }
  return lVisitItems;
};
