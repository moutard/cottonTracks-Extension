...
/**
 * Sticker Each sticker corresponds to a story. They are display in the
 * sticky_bar.
 * 
 * on click : the correponding story is oppened in the story line. on hover :
 * the editable button appear. draggable : drag an sticker in another sticker
 * will merge both stories. droppable :
 */
Cotton.UI.StickyBar.Sticker = Class
    .extend({
      ...
      /**
       * Display the sticker. Handle the UI, and bind to every events.
       */
      display : function() {
        var self = this;
        ...
        // _MERGE.
        $sticker.bind({
          '_merge' : function(event, ui, iSubStoryId) {
        ...
        this._$editable_button = $(
            '<div class="ct-stickers_button_editable"></div>').hide();
       ...
       * @param {int}
       *          [iElastic] : value of an elastic effect when you scroll to
       *          much.
       */
      ...
      /**
       * Check that visitItems id correctly load, if not load it. Then call draw
       * story, to create the storyline.
       * 
       * Bind on event : "click"
       */
      openStory : function(bForceReload) {
        ...
        if (self._oStory.visitItems().length === 0) {
          ...
          DEBUG && console.debug("ct - visitItems already loaded");
          DEBUG && console.debug(self._oStory.visitItems());
      ...
      /**
       * Resize the image so it takes the whole place in the div sticker. Call
       * on the load callback function.
       * 
       * @param {HtmlElement}
       *          $img
       */
      ...
      /**
       * Make a db request to get corresponding visitItems. Called when the
       * sticker is initialized.
       */
      getVisits : function(mCallBackFunction) {
        ...
        var oStore = new Cotton.DB.Store('ct', {
          ...
          oStore.findGroup('visitItems', 'id', self._oStory.visitItemsId(),
              function(lVisitItems) {
                self._oStory.setVisitItems(lVisitItems);
                mCallBackFunction();
              });
      ...
      _remove : function() {
        ...
        if (_oCurrentlyOpenStoryline
            && self._oStory.id() === _oCurrentlyOpenStoryline.story().id()) {
        ...
        self._oBar._lStickers = _.reject(self._oBar._lStickers, function(
            oSticker) {
          return oSticker._iPosition === self._iPosition;
        });
        ...
        _
            .each(
                ...
                function(oSticker) {
                  ...
                  var iLeft = self._iFinalPosition = parseInt(oSticker.$().css(
                      'left'))
                      - Cotton.UI.StickyBar.HORIZONTAL_SPACING;
      ...
      /**
       * Make the stickers completely editable. Including image, title, and
       * remove button.
       * 
      ...
      makeItEditable : function() {
        ...
        if (self._isEditable === false) {
          ...
          $remove_button.mouseup(function() {
            // Ask confirmation.
            var bClear = confirm("Are you sure you want to delete the story "
                + self._oStory.title() + "?\n"
